...
async def detect_dog_face_endpoint(files: List[UploadFile] = File(...)):
    ...
    for file in files:
        ...
        results.append(await detect_dog_face(contents))

    return {"results": results}


@app.post("/detect/cat")
async def detect_cat_face_endpoint(files: List[UploadFile] = File(...)):
    results = []
    ...
    for file in files:
        ...
        await file.seek(0)
        contents = await file.read()
        results.append(await detect_cat_face(contents))
...
async def detect_dog_face(contents):
    # 이미지를 NumPy 배열로 변환
...
async def detect_cat_face(contents):
    # 이미지를 NumPy 배열로 변환
    img = np.asarray(bytearray(contents), dtype="uint8")
    img = cv2.imdecode(img, cv2.IMREAD_COLOR)
